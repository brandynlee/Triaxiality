...
def nfwmodel(xgal,ygal,r200,c,a,b,theta,phi,zl, zs):
	...
	clusterTest = nfw.nfwProfile(parameters={'M':m200, 'c':c, 'a':a, 'b':b, 'theta':theta, 'phi':phi}, z_lens=0.25, mass_definition='200c', cosmology='WMAP7-ML',z_source=1.)

	g1,g2 = clusterTest.reduced_shear(xgal,ygal)
	g1 = g1.flatten()
	g2 = g2.flatten()
...
def lnlike(p, xgal,ygal,eps1,eps2,zl, zs):
	print p
	r200, c, a, b, theta, phi = p
	g1mod,g2mod = nfwmodel(xgal,ygal,r200,c,a,b,theta,phi,zl, zs)
	...
	loglike  = np.sum(logliketemp[np.isfinite(logliketemp)])
...
def lnprob(p, xgal,ygal,eps1,eps2, zl, zs):
	lp = lnprior(p,zl)
...
def MCfit(nwalkers, lenburn, lenchain,xgal, ygal, eps1, eps2, zl,zs, maxlikeReturn = False,):
	rin = 0.5
	rout = 3.
	...
	pos = np.transpose([r200_start, c_start, a_start, b_start, theta_start, phi_start])
	sampler = emcee.EnsembleSampler(nwalkers,ndim,lnprob,args=(xgal,ygal,eps1,eps2,zl,zs), threads=20)
